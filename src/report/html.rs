...
pub fn generate(out: Option<PathBuf>) -> Result<()> {
    ...
    let temp_markdown = workspace.join("reports").join("report.md");
    crate::report::markdown::generate(Some(temp_markdown.clone()))?;
    ...
    let out = out.unwrap_or_else(|| workspace.join("reports").join("report.html"));
    if let Some(parent) = out.parent() {
        fs::create_dir_all(parent)?;
    }
    fs::write(&out, document)?;
    println!("generated HTML report at {}", out.display());
    Ok(())
}
